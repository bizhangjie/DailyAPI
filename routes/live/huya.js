...
const huyaLiveRouter = new Router();
...
// 接口信息
const routerInfo = {
    name: "huyaLive", title: "huyaLive直播", subtitle: "每日榜", category: "直播源"
};

// 缓存键名
const cacheKey = "huyaLiveData";

// 调用时间
let updateTime = new Date().toISOString();

// 永久导航页（需翻墙）
const Host = "https://huya.com";
...
// 播放地址
huyaLiveRouter.get("/huyaLive/:uid", async (ctx) => {
    ...
    console.log(`获取huyaLive播放地址 => ${uid}`);
    ...
    try {
        ...
        if (!data) {
            // 如果缓存中不存在数据
            console.log("从服务端重新获取huyaLive直播播放地址 => " + uid);
            // 从服务器拉取数据
            // 要执行的Python脚本路径
            const pythonScriptPath = '/opt/DailyAPI/py/huya_stream.py';
...
// huyaLive直播搜索
huyaLiveRouter.get("/huyaLive/:wd/:page", async (ctx) => {
    ...
    try {
        ...
        if (!data) {
            // 如果缓存中不存在数据
            console.log("从服务端重新获取huyaLive直播");
            // 从服务器拉取数据
...
huyaLiveRouter.info = routerInfo;
module.exports = huyaLiveRouter;

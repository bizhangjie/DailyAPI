...
const douyuLiveRouter = new Router();
...
// 接口信息
const routerInfo = {
    name: "douyuLive", title: "douyuLive直播", subtitle: "每日榜", category: "直播源"
};

// 缓存键名
const cacheKey = "douyuLiveData";

// 调用时间
let updateTime = new Date().toISOString();

// 永久导航页（需翻墙）
const Host = "https://douyu.com";
...
// 播放地址
douyuLiveRouter.get("/douyuLive/:uid", async (ctx) => {
    ...
    console.log(`获取douyuLive播放地址 => ${uid}`);
    ...
    try {
        ...
        if (!data) {
            // 如果缓存中不存在数据
            console.log("从服务端重新获取douyuLive直播播放地址 => " + uid);
            // 从服务器拉取数据
            // 要执行的Python脚本路径
            const pythonScriptPath = '/opt/DailyAPI/py/douyu_stream.py';
...
// douyuLive直播搜索
douyuLiveRouter.get("/douyuLive/:wd/:page", async (ctx) => {
    ...
    try {
        ...
        if (!data) {
            // 如果缓存中不存在数据
            console.log("从服务端重新获取douyuLive直播");
            // 从服务器拉取数据
...
douyuLiveRouter.info = routerInfo;
module.exports = douyuLiveRouter;
